$(function () {
	if (
		$(".js-trans-somv-inprogress").length > 0
	) {
		$.ajax({
            ...
            $th_date_result.val(data[0][0].date_result);
            $th_notes_verify.val(data[0][0].notes);
					}, 100);
				}, 100);

				$.ajax({
					url: URL + "asyncsom/transacation_data_thid?th_id=" + data[0][0].ref_doc,
					success: function (datax) { 
						$('.js-ref-doc').val(datax[0][0].trans_code);
					},
					error: function (err) {
						console.log(err);
					}
		...
		// submission listener
		$("#somvInprogressSubmit button").on("click", function (e) {
			e.preventDefault();
			if ($(this).attr("name") == "save") {
				...
				let serialize = {
					...
					trans_code: $.trim($("#somvInprogressSubmit").find('input[name="trans_code"]').val())
				};
				console.log(somValidation().length)
